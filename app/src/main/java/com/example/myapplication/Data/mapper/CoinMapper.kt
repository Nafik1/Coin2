...
class CoinMapper {
    fun mapDtotoDbModel(dto : CoinInfoDto) : CoinInfoDBModel {
        return CoinInfoDBModel(
            ...
            imageurl = IMAGEBASE_URL + dto.imageurl
        )
    }
    fun mapJsonContainerToListCoinInfo(jsoncontrainer : CoinInfoJsonContainerDto) : List<CoinInfoDto> {
        val result = mutableListOf<CoinInfoDto>()
        val jsonObject = jsoncontrainer.coinPriceInfoJsonObject
        if (jsonObject == null) {
            return result
        }
